...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    float gray = dot(buf[0].rgb, vec3(0.299, 0.587, 0.114));

    // Vibrant brand blues
    vec3 deepBlue = vec3(0.114, 0.306, 0.847);   // #1d4ed8
    vec3 primaryBlue = vec3(0.231, 0.510, 0.965); // #3b82f6
    vec3 brightBlue = vec3(0.376, 0.647, 0.980);  // #60a5fa
    vec3 skyBlue = vec3(0.220, 0.647, 0.914);     // #38bdf8

    // Horizontal position factor - fade to gray on left and right edges
    float horizontalFade = abs(coordinate.x);
    float edgeGray = smoothstep(0.3, 0.9, horizontalFade);

    // Blue intensity - reduced at horizontal edges
    float blueIntensity = smoothstep(0.25, 0.6, buf[0].x) * 0.85 * (1.0 - edgeGray * 0.8);

    // Dynamic blend between vibrant blues
    vec3 blueAccent = mix(
      mix(deepBlue, primaryBlue, buf[0].y),
      mix(brightBlue, skyBlue, buf[0].z),
      buf[0].x
    );

    // Blend gray with vibrant blue - more gray at edges
    vec3 grayBase = vec3(gray);
    vec3 finalColor = mix(grayBase, blueAccent * (gray + 0.5), blueIntensity);

    // Boost saturation and vibrancy
    finalColor = pow(finalColor, vec3(0.8));
    finalColor = finalColor * 1.15; // Brightness boost
...
function ShaderPlane() {
  ...
  useFrame((state) => {
    ...
    // Dynamically scale plane to cover viewport
    if (meshRef.current) {
      const aspect = width / height;
      // Scale to ensure full coverage regardless of aspect ratio
      const scale = Math.max(aspect, 1) * 2.5;
